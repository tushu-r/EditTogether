      ...
      '>{activeTeam?.teamName}
            ...
            <PopoverContent className='ml-7 p-4'>
               {/* Team Section  */}
                <div>
                    {teamList?.map((team,index)=>(
                        ...
                        className={`p-2 
                         ...
                         ${activeTeam?._id == team._id ? 'bg-blue-500 hover:bg-blue-500 text-white' : 'hover:bg-gray-500'}`}
                         onClick={()=>setActiveTeam(team)}
                        >{team.teamName}</h2>
                    ))}
                    
                </div>
                <Separator className='mt-2 bg-slate-100 h-[5px]'/>
                {/* Option Section  */}
                <div>
                    {menu.map((item,index)=>(
                ...
                <Separator className='mt-2 h-[5px] bg-slate-100'/>
                {/* User Info Section  */}
